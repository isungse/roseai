...
- 본문 강조는 슬라이드 JSON 의 선택 키 `accent` (본문의 정확한 부분 문자열). `RevealText` 가 문자 범위로 `text-brand` 를 입혀 조사가 붙은 단어("네이티브로.")에서도 구절만 붉게 남는다. 레드는 핵심 구절 하나에만 — 줄 전체를 칠하면 제목처럼 보여 다른 슬라이드와 형식이 어긋난다.
